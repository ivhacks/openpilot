...
from openpilot.selfdrive.modeld.park_nav import MAX_RIGHT_CURVATURE, NAV_MAX_SPEED, NavDesireInjector
...
def get_action_from_model(model_output: dict[str, np.ndarray], prev_action: log.ModelDataV2.Action,
                          ...
                          desire=log.Desire.none, nav_active: bool = False) -> log.ModelDataV2.Action:
  ...
  if MIN_LAT_CONTROL_SPEED < v_ego < NAV_MAX_SPEED and desire == log.Desire.turnLeft:
  ...
  # Summon only ever turns left. A right curvature request while navigating is the
  # model misreading the lot, so cap it at what a straight line needs.
  if nav_active:
    desired_curvature = min(desired_curvature, MAX_RIGHT_CURVATURE)
...
def main(demo=False):
  ...
  while True:
    ...
    # blinker + parking-nav (parknavd) turn desires
    DH.update(sm['carState'], sm['carControl'].latActive)
    nav_alive = sm.alive['parkNavSignal']
    desire = nav_injector.update(sm['parkNavSignal'], nav_alive, sm['carState'],
                                 sm['carControl'].latActive, v_ego, DH.desire)
